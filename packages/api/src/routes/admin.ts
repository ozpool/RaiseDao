 ...
 *  trust signals: list risk-scored campaigns, hide/un-hide them from public
 *  browse, grant/revoke the verified badge, and read the audit trail. There is
 *  deliberately NO route that can move funds or change votes — those live only
 *  on-chain, behind the governor and timelock.
...
export function adminRouter(store: AdminStore): Router {
  ...
  router.post('/admin/campaigns/:vault/verify', ...admin, async (req: Request, res: Response) => {
    ...
    const actor = req.user!.address;
    const vault = req.params.vault ?? '';
    const updated = await store.setVerified(
      vault,
      verified,
      actor,
      typeof reason === 'string' ? reason : '',
